...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const result = await sql`SELECT password FROM users WHERE username = 'Al-Azhar' LIMIT 1`;
      return res.status(200).json({ password: result[0]?.password || 'Azhar6677' });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (masterOtp === '2026') {
        await sql`UPDATE users SET password = ${newPassword} WHERE username = 'Al-Azhar'`;
